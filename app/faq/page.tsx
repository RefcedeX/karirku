      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-6 py-12 lg:py-20 relative">
        ...
        >
          ...
          <motion.h1 variants={fadeIn} className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-slate-900 mb-6 leading-tight">
            Tentang & <span className="text-blue-700">FAQ</span>
          </motion.h1>
        ...
        >
          {faqs.map((faq, index) => (
            <motion.div 
              key={index} 
              variants={fadeIn}
              className={`bg-white rounded-2xl overflow-hidden transition-all duration-300 border ${openIndex === index ? 'border-blue-700 ring-4 ring-blue-700/10 shadow-lg shadow-blue-700/10' : 'border-slate-100 shadow-sm hover:border-slate-200 hover:shadow-md'}`}
            >
              <button 
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className="w-full px-6 py-5 text-left flex items-center justify-between gap-4"
              >
                <span className={`font-bold text-lg ${openIndex === index ? 'text-blue-800' : 'text-slate-800'}`}>
                  {faq.question}
                </span>
                <ChevronDown 
                  size={24} 
                  className={`shrink-0 transition-transform duration-300 ${openIndex === index ? 'rotate-180 text-blue-700' : 'text-slate-400'}`} 
                />
              </button>
